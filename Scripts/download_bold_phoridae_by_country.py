#!/usr/bin/env python3
"""Download BOLD Phoridae records country by country.

Phoridae is one of the four over-cap Diptera families. BOLD summary metadata
shows every country-level Phoridae query is below the 1M-record cap, so this
...
FAMILY = "Phoridae"
DEFAULT_OUTDIR = PROJECT_ROOT / "Data" / "raw" / "bold" / "diptera_phoridae_by_country"
FAMILY_SUMMARY_PATH = (
    PROJECT_ROOT
    / "Data"
    / "raw"
    / "bold"
    / "diptera_by_family"
    / "bold_global_diptera_family_phoridae_oversized_summary.json"
)
DEFAULT_RETRIES = 3
...
def download_country(country_or_ocean: str, outdir: Path, fmt: str, timeout: int) -> None:
    ...
    stem = f"bold_global_diptera_family_phoridae_country_{slug(country_or_ocean)}"
...
def main() -> int:
    ...
    manifest_path = args.outdir / "phoridae_country_manifest.csv"
    summary_path = args.outdir / "phoridae_country_download_summary.csv"
    failed_path = args.outdir / "phoridae_country_failed_downloads.csv"
    ...
    for row in countries:
        ...
        stem = f"bold_global_diptera_family_phoridae_country_{slug(country)}"
