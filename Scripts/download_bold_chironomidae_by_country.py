#!/usr/bin/env python3
"""Download BOLD Chironomidae records country by country.

Chironomidae is one of the four over-cap Diptera families. BOLD summary
metadata shows every country-level Chironomidae query is below the 1M-record
cap, so this script downloads one TSV per country/ocean value.
"""
...
FAMILY = "Chironomidae"
DEFAULT_OUTDIR = PROJECT_ROOT / "Data" / "raw" / "bold" / "diptera_chironomidae_by_country"
FAMILY_SUMMARY_PATH = (
    PROJECT_ROOT
    / "Data"
    / "raw"
    / "bold"
    / "diptera_by_family"
    / "bold_global_diptera_family_chironomidae_oversized_summary.json"
)
DEFAULT_RETRIES = 3
...
def country_rows_from_summary(summary: dict) -> list[dict[str, object]]:
    country_counts = summary.get("country/ocean", {}) or {}
    rows = [
        {"country_or_ocean": country, "records_family_summary": int(count)}
        for country, count in country_counts.items()
        if country and int(count) > 0
    ]
...
def download_country(country_or_ocean: str, outdir: Path, fmt: str, timeout: int) -> None:
    ...
    stem = f"bold_global_diptera_family_chironomidae_country_{slug(country_or_ocean)}"
...
def main() -> int:
    ...
    manifest_path = args.outdir / "chironomidae_country_manifest.csv"
    summary_path = args.outdir / "chironomidae_country_download_summary.csv"
    failed_path = args.outdir / "chironomidae_country_failed_downloads.csv"

    family_summary = load_or_fetch_family_summary(args.timeout, args.refresh_family_summary)
    countries = country_rows_from_summary(family_summary)
    ...
    for row in countries:
        ...
        stem = f"bold_global_diptera_family_chironomidae_country_{slug(country)}"
