#!/usr/bin/env python3
"""Download BOLD Sciaridae records country by country.

Sciaridae is one of the four over-cap Diptera families. BOLD summary metadata
shows every country-level Sciaridae query is below the 1M-record cap, so this
script downloads one TSV per country/ocean value.
"""
...
FAMILY = "Sciaridae"
DEFAULT_OUTDIR = PROJECT_ROOT / "Data" / "raw" / "bold" / "diptera_sciaridae_by_country"
FAMILY_SUMMARY_PATH = (
    PROJECT_ROOT
    / "Data"
    / "raw"
    / "bold"
    / "diptera_by_family"
    / "bold_global_diptera_family_sciaridae_oversized_summary.json"
)
DEFAULT_RETRIES = 3
...
def download_country(country_or_ocean: str, outdir: Path, fmt: str, timeout: int) -> None:
    ...
    stem = f"bold_global_diptera_family_sciaridae_country_{slug(country_or_ocean)}"
...
def main() -> int:
    ...
    manifest_path = args.outdir / "sciaridae_country_manifest.csv"
    summary_path = args.outdir / "sciaridae_country_download_summary.csv"
    failed_path = args.outdir / "sciaridae_country_failed_downloads.csv"
    ...
    for row in countries:
        ...
        stem = f"bold_global_diptera_family_sciaridae_country_{slug(country)}"
