#!/usr/bin/env python3
"""Download Coleoptera from BOLD family by family.

This avoids the 1M-record query cap by scraping the BOLD v4 taxonomy browser
for Coleoptera family names, then issuing one BOLD Portal query per family.
...
ORDER = "Coleoptera"
ORDER_TAXID = "413"
DEFAULT_OUTDIR = PROJECT_ROOT / "Data" / "raw" / "bold" / "coleoptera_by_family"
DEFAULT_RETRY_SLEEP = 61
DEFAULT_BETWEEN_FAMILY_SLEEP = 21
DEFAULT_RETRIES = 3
DEFAULT_TIMEOUT = 180
...
def scrape_coleoptera_families(timeout: int) -> list[dict[str, object]]:
    url = f"https://v4.boldsystems.org/index.php/Taxbrowser_Taxonpage?taxid={ORDER_TAXID}"
    text = get_text(url, timeout)
    section = re.search(r"Families \(\d+\).*?</ol>", text, flags=re.S)
    if not section:
        raise RuntimeError("Could not find Coleoptera Families section on BOLD v4 page")

    rows = []
    for taxid, family, count in re.findall(
        r'href="\?taxid=(\d+)">([^<\[]+?) \[(\d+)\]</a>',
        section.group(0),
    ):
        rows.append(
...
def download_family(family: str, outdir: Path, fmt: str, timeout: int) -> None:
    ...
    stem = f"bold_global_coleoptera_family_{slug(family)}"
    ...
    print(f"{family}: downloading to {output_path}")
...
def main() -> int:
    ...
    manifest_path = args.outdir / "coleoptera_family_manifest.csv"
    summary_path = args.outdir / "coleoptera_family_download_summary.csv"
    failed_path = args.outdir / "coleoptera_family_failed_downloads.csv"

    families = scrape_coleoptera_families(args.timeout)
    ...
    for row in families:
        ...
        try:
            ...
            print(f"{family}: {n_records:,} records; {n_coord:,} coords ({coord_share:.1f}%)")
            ...
            print(f"{family}: failed after retries: {exc}", file=sys.stderr)
            append_csv(
                failed_path,
                {
                    "family": family,
