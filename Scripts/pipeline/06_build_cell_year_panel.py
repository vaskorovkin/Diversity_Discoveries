...
def load_land_cells(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            f"Missing land-cell file: {path}. Run Scripts/pipeline/05_cell_correlations.py first."
        )
    ...
    if freq == "quarter":
        ...
        if "collection_month" not in minimal_head.columns:
            raise ValueError(
                ...
                "Rebuild the minimal file with Scripts/pipeline/00_build_bold_minimal.py first."
            )
