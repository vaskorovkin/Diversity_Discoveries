#!/usr/bin/env python3
"""Download Hymenoptera from BOLD family by family.

This avoids the 1M-record query cap by scraping the BOLD v4 taxonomy browser
for Hymenoptera family names, then issuing one BOLD Portal query per family.
...
ORDER = "Hymenoptera"
DEFAULT_OUTDIR = PROJECT_ROOT / "Data" / "raw" / "bold" / "hymenoptera_by_family"
...
def scrape_hymenoptera_families(timeout: int) -> list[dict[str, object]]:
    ...
    if not section:
        raise RuntimeError("Could not find Hymenoptera Families section on BOLD v4 page")
...
def download_family(family: str, outdir: Path, fmt: str, timeout: int) -> None:
    ...
    stem = f"bold_global_hymenoptera_family_{slug(family)}"
...
def main() -> int:
    ...
    manifest_path = args.outdir / "hymenoptera_family_manifest.csv"
    summary_path = args.outdir / "hymenoptera_family_download_summary.csv"
    failed_path = args.outdir / "hymenoptera_family_failed_downloads.csv"

    families = scrape_hymenoptera_families(args.timeout)
