#!/usr/bin/env python3
"""Download manageable Diptera families from BOLD family by family.

This starts at Ceratopogonidae by default, skipping the four largest Diptera
families that exceed the 1M-record BOLD query cap and need genus-level splits:
Cecidomyiidae, Chironomidae, Phoridae, and Sciaridae.
"""
...
ORDER = "Diptera"
DEFAULT_OUTDIR = PROJECT_ROOT / "Data" / "raw" / "bold" / "diptera_by_family"
DEFAULT_START_FAMILY = "Ceratopogonidae"
...
def scrape_diptera_families(timeout: int) -> list[dict[str, object]]:
    ...
    if not section:
        raise RuntimeError("Could not find Diptera Families section on BOLD v4 page")
...
def slice_from_family(families: list[dict[str, object]], start_family: str | None) -> list[dict[str, object]]:
    if not start_family:
        return families
    for index, row in enumerate(families):
        if row["family"] == start_family:
            return families[index:]
    raise RuntimeError(f"Could not find start family in Diptera manifest: {start_family}")
...
def download_family(family: str, outdir: Path, fmt: str, timeout: int) -> None:
    ...
    stem = f"bold_global_diptera_family_{slug(family)}"
...
def main() -> int:
    ...
    parser.add_argument("--start-family", default=DEFAULT_START_FAMILY)
    parser.add_argument(
        "--max-consecutive-403",
        type=int,
    ...
    manifest_path = args.outdir / "diptera_family_manifest.csv"
    summary_path = args.outdir / "diptera_family_download_summary.csv"
    failed_path = args.outdir / "diptera_family_failed_downloads.csv"

    families = scrape_diptera_families(args.timeout)
    ...
    if args.families:
        ...
        families = slice_from_family(families, args.start_family)
