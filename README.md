...
- BIEN should not be queried cell by cell for the full 100 km grid. `Scripts/19_extract_gbif_plantae_species_universe.py` first builds the observed GBIF preserved/material plant species universe, and `Scripts/18_bien_range_download_pilot.R` then takes the top-N species from that ranked universe (default 5000), checks BIEN range availability, downloads BIEN range shapefiles locally in bulk, records timing/disk-footprint metrics, and can optionally convert them to BIEN skinny ranges plus a local richness raster.
