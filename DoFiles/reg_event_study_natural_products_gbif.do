* reg_event_study_natural_products_gbif.do
* Conflict -> GBIF Plantae natural-products discovery outcomes
* Uses the primary pair:
*   - gp_np_any
*   - gp_np_log
...
capture log close
log using "$proj/Logs/reg_event_study_natural_products_gbif.log", replace text
...
* -------------------------------------------------------------------
* Load GBIF Plantae natural-products outcomes and merge to master panel
* -------------------------------------------------------------------
...
foreach v in gp_np_species gp_total_species gp_unique_compounds gp_records {
...
summarize gp_np_any gp_np_log gp_np_species gp_unique_compounds gp_total_species
...
local est_twfe_any      "twfe_anynp"
local est_twfe_log      "twfe_lognp"
local est_bjs_any       "bjs_anynp"
local est_bjs_log       "bjs_lognp"
local est_cont          "cont_lognp"
local est_dcdh          "dcdh_lognp"
local est_cs            "cs_lognp"
local est_twfe_conflict "twfe_c_lognp"
local est_twfe_drought  "twfe_d_lognp"
local est_twfe_fire     "twfe_f_lognp"
local est_bjs_drought   "bjs_d_lognp"
local est_bjs_fire      "bjs_f_lognp"
...
display _n "{txt}=== STEP 1: TWFE event-study (NP outcomes) ==="
...
display _n "{txt}=== STEP 3: DL continuous DID (log NP outcomes) ==="
...
estadd local lhs_label    "gp_np_log"

* -------------------------------------------------------------------
* Step 3b: dCDH continuous DID
* -------------------------------------------------------------------

display _n "{txt}=== STEP 3b: dCDH dynamic DID (log NP outcomes) ==="

if "`fe_mode'" == "rich" {
    capture drop gp_np_log_resid
    qui reghdfe gp_np_log, absorb(`absorb_main') resid(gp_np_log_resid)
    qui did_multiplegt_dyn gp_np_log_resid cell_id_num year log1p_conflict, ///
        effects(10) placebo(5) cluster(cell_id_num) graph_off
}
else {
    qui did_multiplegt_dyn gp_np_log cell_id_num year log1p_conflict, ///
        effects(10) placebo(5) cluster(cell_id_num) graph_off
}
...
display _n "{txt}=== Callaway-Sant'Anna robustness (log NP outcomes) ==="

capture drop gvar_conflict
gen gvar_conflict = cond(missing(Ei_conflict), 0, Ei_conflict)

qui csdid gp_np_log, ivar(cell_id_num) time(year) gvar(gvar_conflict) ///
    method(reg) notyet
...
* -------------------------------------------------------------------
* Multi-shock TWFE / BJS on log NP outcomes
* -------------------------------------------------------------------
...
foreach shk in drought fire {
    qui did_imputation gp_np_log cell_id_num year Ei_`shk', ///
        fe(`did_fe') horizons(0/8) pretrends(5) ///
        cluster(cell_id_num) autosample
...
esttab `est_twfe_any' `est_bjs_any' `est_twfe_log' `est_bjs_log', ///
    ...
    title("Table 1: GBIF Plantae NP conflict identification ladder (window -6/+8)") ///
    mtitles("gp_np_any" "gp_np_any" "gp_np_log" "gp_np_log") ///
...
esttab `est_cont' `est_dcdh' `est_cs', ///
    ...
    title("Table 2: GBIF Plantae NP continuous DID + CS robustness") ///
...
esttab `est_bjs_log' `est_bjs_drought' `est_bjs_fire', ///
    ...
    stats(att_avg_03_txt att_avg_03_se_txt ///
          ...
          labels("Avg ATT tau0-tau2" " " ///
                 ...
                 "Cell FE" "Year FE" "Country x Year FE" "Biome x Year FE") ///
          fmt(%s %s %s %s %s %s %9.4f %s %s %s %9.0fc %9.4f %s %s %s %s)) ///
    title("Table 3: GBIF Plantae NP multi-shock BJS comparison (window -6/+8)") ///
...
esttab `est_twfe_conflict' `est_bjs_log' ///
    ...
    using "$DD_CODEX_TABLES/tab_event_study_np_twfe_bjs.tex", ///
...
esttab `est_twfe_conflict' `est_twfe_drought' `est_twfe_fire', ///
    ...
    stats(att_avg_03_txt att_avg_03_se_txt ///
          ...
          labels("Avg ATT tau0-tau2" " " ///
                 ...
                 "Cell FE" "Year FE" "Country x Year FE" "Biome x Year FE") ///
          fmt(%s %s %s %s %s %s %9.4f %s %s %s %9.0fc %9.4f %s %s %s %s)) ///
    title("Table 4: GBIF Plantae NP multi-shock TWFE comparison (window -6/+8)") ///
    mtitles("Conflict" "Severe drought" "Fire") ///
    compress
...
coefplot (`est_bjs_log', label("BJS imputation") msymbol(D) mcolor(maroon) ciopts(lcolor(maroon))), ///
    ...
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_np_bjs_log, replace)

graph combine es_np_twfe_log es_np_bjs_log, ///
    title("TWFE vs BJS: conflict effect on GBIF Plantae NP discovery (intensive)", size(medium)) ///
    cols(2) ycommon name(es_np_conflict_log, replace)
graph save "$proj/Output/figures/event_study/twfe_vs_bjs_conflict_np_log1p.gph", replace
graph export "$proj/Output/figures/event_study/twfe_vs_bjs_conflict_np_log1p.pdf", replace
graph export "$codex_figures/twfe_vs_bjs_conflict_np_log1p.pdf", replace
...
coefplot (`est_twfe_any', label("TWFE event-study") msymbol(O) mcolor(navy) ciopts(lcolor(navy))), ///
    ...
    vertical yline(0, lpattern(dash) lcolor(black)) ///
    xtitle("Years from conflict onset") ///
    ytitle("Effect on 1[NP species > 0]") ///
    title("TWFE event-study", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_np_twfe_any, replace)
...
coefplot (`est_bjs_any', label("BJS imputation") msymbol(D) mcolor(maroon) ciopts(lcolor(maroon))), ///
    ...
    title("BJS imputation", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_np_bjs_any, replace)

graph combine es_np_twfe_any es_np_bjs_any, ///
    title("TWFE vs BJS: conflict effect on GBIF Plantae NP discovery (extensive)", size(medium)) ///
    cols(2) ycommon name(es_np_conflict_any, replace)
graph save "$proj/Output/figures/event_study/twfe_vs_bjs_conflict_np_any.gph", replace
graph export "$proj/Output/figures/event_study/twfe_vs_bjs_conflict_np_any.pdf", replace
graph export "$codex_figures/twfe_vs_bjs_conflict_np_any.pdf", replace
...
coefplot ///
    ...
    ytitle("Effect on log(1 + NP species)") ///
    title("BJS event-study: GBIF Plantae NP discovery response to shock onsets", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(rows(1) position(6) size(small)) ///
    name(es_np_multishock_bjs, replace)
graph save "$proj/Output/figures/event_study/bjs_conflict_drought_fire_np.gph", replace
graph export "$proj/Output/figures/event_study/bjs_conflict_drought_fire_np.pdf", replace
graph export "$codex_figures/bjs_conflict_drought_fire_np.pdf", replace
...
coefplot ///
    ...
    xtitle("Years from shock onset (tau_-1 = reference)") ///
    ytitle("Effect on log(1 + NP species)") ///
    title("TWFE event-study: GBIF Plantae NP discovery response to shock onsets", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(rows(1) position(6) size(small)) ///
    name(es_np_multishock_twfe, replace)
graph save "$proj/Output/figures/event_study/twfe_conflict_drought_fire_np.gph", replace
graph export "$proj/Output/figures/event_study/twfe_conflict_drought_fire_np.pdf", replace
graph export "$codex_figures/twfe_conflict_drought_fire_np.pdf", replace
...
coefplot (`est_cont', label("DL continuous") msymbol(O) mcolor(navy) ciopts(lcolor(navy))), ///
    ...
    ytitle("Effect on log(1 + NP species) per unit log(1 + events)") ///
    title("Continuous-treatment DL DID: conflict intensity and NP discovery", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_np_continuous, replace)
graph save "$proj/Output/figures/event_study/continuous_dl_conflict_np_log1p.gph", replace
graph export "$proj/Output/figures/event_study/continuous_dl_conflict_np_log1p.pdf", replace
graph export "$codex_figures/continuous_dl_conflict_np_log1p.pdf", replace
...
coefplot (`est_dcdh', label("dCDH dynamic") msymbol(O) mcolor(navy) ciopts(lcolor(navy))), ///
    ...
    ytitle("Effect on log(1 + NP species) per unit log(1 + events)") ///
    title("dCDH continuous DID: conflict intensity and NP discovery", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_np_dcdh, replace)
graph save "$proj/Output/figures/event_study/dcdh_continuous_conflict_np_log1p.gph", replace
graph export "$proj/Output/figures/event_study/dcdh_continuous_conflict_np_log1p.pdf", replace
graph export "$codex_figures/dcdh_continuous_conflict_np_log1p.pdf", replace

display _n "{txt}=== Done. ==="
display "Log:    $proj/Logs/reg_event_study_natural_products_gbif.log"
display "Figures:"
display "  $proj/Output/figures/event_study/twfe_vs_bjs_conflict_np_log1p.pdf"
display "  $proj/Output/figures/event_study/twfe_vs_bjs_conflict_np_any.pdf"
display "  $proj/Output/figures/event_study/bjs_conflict_drought_fire_np.pdf"
display "  $proj/Output/figures/event_study/twfe_conflict_drought_fire_np.pdf"
display "  $proj/Output/figures/event_study/continuous_dl_conflict_np_log1p.pdf"
display "  $proj/Output/figures/event_study/dcdh_continuous_conflict_np_log1p.pdf"
