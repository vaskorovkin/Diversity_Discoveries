* reg_event_study_bin_new.do
* Conflict -> BIN new-discovery outcomes event studies
* Mirrors reg_event_study.do, but uses:
*   - any_n_new_bins
*   - log1p_n_new_bins
...
capture log close
log using "$proj/Logs/reg_event_study_bin_new.log", replace text
...
summarize any_n_new_bins log1p_n_new_bins n_new_bins
...
local est_twfe_any      "twfe_anynew"
local est_twfe_log      "twfe_lognew"
local est_bjs_any       "bjs_anynew"
local est_bjs_log       "bjs_lognew"
local est_cont          "cont_lognew"
local est_dcdh          "dcdh_lognew"
local est_cs            "cs_lognew"
local est_twfe_conflict "twfe_c_lognew"
local est_twfe_drought  "twfe_d_lognew"
local est_twfe_fire     "twfe_f_lognew"
local est_bjs_drought   "bjs_d_lognew"
local est_bjs_fire      "bjs_f_lognew"
...
display _n "{txt}=== STEP 1: TWFE event-study (BIN new outcomes) ==="
...
foreach lhs in any_n_new_bins log1p_n_new_bins {
    qui reghdfe `lhs' ///
        lead6 lead5 lead4 lead3 lead2 ///
        lag0 lag1 lag2 lag3 lag4 lag5 lag6 lag7 lag8, ///
        absorb(`absorb_main') ///
        vce(cluster cell_id_num)
    if "`lhs'" == "any_n_new_bins" estimates store `est_twfe_any'
...
display _n "{txt}=== STEP 2: BJS event-study (BIN new outcomes) ==="

foreach lhs in any_n_new_bins log1p_n_new_bins {
    qui did_imputation `lhs' cell_id_num year Ei_conflict, ///
        fe(`did_fe') horizons(0/8) pretrends(5) ///
        cluster(cell_id_num) autosample
    if "`lhs'" == "any_n_new_bins" estimates store `est_bjs_any'
...
display _n "{txt}=== STEP 3: DL continuous DID (log1p_n_new_bins) ==="
...
estadd local lhs_label    "log1p_n_new_bins"

* -------------------------------------------------------------------
* Step 3b: dCDH continuous DID
* -------------------------------------------------------------------

display _n "{txt}=== STEP 3b: dCDH dynamic DID (log1p_n_new_bins) ==="

if "`fe_mode'" == "rich" {
    capture drop log1p_n_new_bins_resid
    qui reghdfe log1p_n_new_bins, absorb(`absorb_main') resid(log1p_n_new_bins_resid)
    qui did_multiplegt_dyn log1p_n_new_bins_resid cell_id_num year log1p_conflict, ///
        effects(10) placebo(5) cluster(cell_id_num) graph_off
}
else {
    qui did_multiplegt_dyn log1p_n_new_bins cell_id_num year log1p_conflict, ///
        effects(10) placebo(5) cluster(cell_id_num) graph_off
}
...
display _n "{txt}=== Callaway-Sant'Anna robustness (log1p_n_new_bins) ==="

capture drop gvar_conflict
gen gvar_conflict = cond(missing(Ei_conflict), 0, Ei_conflict)

qui csdid log1p_n_new_bins, ivar(cell_id_num) time(year) gvar(gvar_conflict) ///
    method(reg) notyet
...
* -------------------------------------------------------------------
* Multi-shock TWFE / BJS on log1p_n_new_bins
* -------------------------------------------------------------------
...
foreach shk in drought fire {
    qui did_imputation log1p_n_new_bins cell_id_num year Ei_`shk', ///
        fe(`did_fe') horizons(0/8) pretrends(5) ///
        cluster(cell_id_num) autosample
...
esttab `est_twfe_any' `est_bjs_any' `est_twfe_log' `est_bjs_log', ///
    ...
    keep(t_m6 t_m5 t_m4 t_m3 t_m2 t_p0 t_p1 t_p2 t_p3 t_p4 t_p5 t_p6 t_p7 t_p8) ///
    order(t_m6 t_m5 t_m4 t_m3 t_m2 t_p0 t_p1 t_p2 t_p3 t_p4 t_p5 t_p6 t_p7 t_p8) ///
    se star(* 0.10 ** 0.05 *** 0.01) b(4) se(4) ///
    varlabels(t_m6 "tau_-6+" t_m5 "tau_-5" t_m4 "tau_-4" t_m3 "tau_-3" t_m2 "tau_-2" ///
    ...
    title("Table 1: BIN new outcomes conflict identification ladder (window -6/+8)") ///
    mtitles("any_n_new_bins" "any_n_new_bins" "log1p_n_new_bins" "log1p_n_new_bins") ///
...
esttab `est_cont' `est_dcdh' `est_cs', ///
    ...
    title("Table 2: BIN new outcomes continuous DID + CS robustness") ///
...
esttab `est_bjs_log' `est_bjs_drought' `est_bjs_fire', ///
    ...
    stats(att_avg_03_txt att_avg_03_se_txt ///
          ...
          labels("Avg ATT tau0-tau2" " " ///
                 ...
                 "Cell FE" "Year FE" "Country x Year FE" "Biome x Year FE") ///
          fmt(%s %s %s %s %s %s %9.4f %s %s %s %9.0fc %9.4f %s %s %s %s)) ///
    title("Table 3: BIN new outcomes multi-shock BJS comparison (window -6/+8)") ///
...
esttab `est_twfe_conflict' `est_bjs_log' ///
    ...
    using "$DD_CODEX_TABLES/tab_event_study_bin_twfe_bjs.tex", ///
...
esttab `est_twfe_conflict' `est_twfe_drought' `est_twfe_fire', ///
    keep(lead6 lead5 lead4 lead3 lead2 lag0 lag1 lag2 lag3 lag4 lag5 lag6 lag7 lag8) ///
    order(lead6 lead5 lead4 lead3 lead2 lag0 lag1 lag2 lag3 lag4 lag5 lag6 lag7 lag8) ///
    se star(* 0.10 ** 0.05 *** 0.01) b(4) se(4) ///
    varlabels(lead6 "tau_-6+" lead5 "tau_-5" lead4 "tau_-4" lead3 "tau_-3" lead2 "tau_-2" ///
    ...
    stats(att_avg_03_txt att_avg_03_se_txt ///
          ...
          labels("Avg ATT tau0-tau2" " " ///
                 ...
                 "Cell FE" "Year FE" "Country x Year FE" "Biome x Year FE") ///
          fmt(%s %s %s %s %s %s %9.4f %s %s %s %9.0fc %9.4f %s %s %s %s)) ///
    title("Table 4: BIN new outcomes multi-shock TWFE comparison (window -6/+8)") ///
    mtitles("Conflict" "Severe drought" "Fire") ///
    compress
...
coefplot (`est_bjs_log', label("BJS imputation") msymbol(D) mcolor(maroon) ciopts(lcolor(maroon))), ///
    ...
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_binnew_bjs_log, replace)

graph combine es_binnew_twfe_log es_binnew_bjs_log, ///
    title("TWFE vs BJS: conflict effect on BIN new discoveries (intensive)", size(medium)) ///
    cols(2) ycommon name(es_binnew_conflict_log, replace)
graph save "$proj/Output/figures/event_study/twfe_vs_bjs_conflict_bin_new_log1p.gph", replace
graph export "$proj/Output/figures/event_study/twfe_vs_bjs_conflict_bin_new_log1p.pdf", replace
graph export "$codex_figures/twfe_vs_bjs_conflict_bin_new_log1p.pdf", replace
...
coefplot (`est_twfe_any', label("TWFE event-study") msymbol(O) mcolor(navy) ciopts(lcolor(navy))), ///
    ...
    vertical yline(0, lpattern(dash) lcolor(black)) ///
    xtitle("Years from conflict onset") ///
    ytitle("Effect on 1[new BINs > 0]") ///
    title("TWFE event-study", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_binnew_twfe_any, replace)
...
coefplot (`est_bjs_any', label("BJS imputation") msymbol(D) mcolor(maroon) ciopts(lcolor(maroon))), ///
    ...
    title("BJS imputation", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_binnew_bjs_any, replace)

graph combine es_binnew_twfe_any es_binnew_bjs_any, ///
    title("TWFE vs BJS: conflict effect on BIN new discoveries (extensive)", size(medium)) ///
    cols(2) ycommon name(es_binnew_conflict_any, replace)
graph save "$proj/Output/figures/event_study/twfe_vs_bjs_conflict_bin_new_any.gph", replace
graph export "$proj/Output/figures/event_study/twfe_vs_bjs_conflict_bin_new_any.pdf", replace
graph export "$codex_figures/twfe_vs_bjs_conflict_bin_new_any.pdf", replace

* -------------------------------------------------------------------
* Figure 3
* -------------------------------------------------------------------

local plotcoefs pre5 pre4 pre3 pre2 pre1 tau0 tau1 tau2 tau3 tau4 tau5 tau6 tau7 tau8
...
coefplot ///
    ...
    ytitle("Effect on log(1 + new BINs)") ///
    title("BJS event-study: BIN new-discovery response to shock onsets", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(rows(1) position(6) size(small)) ///
    name(es_binnew_multishock_bjs, replace)
graph save "$proj/Output/figures/event_study/bjs_conflict_drought_fire_bin_new.gph", replace
graph export "$proj/Output/figures/event_study/bjs_conflict_drought_fire_bin_new.pdf", replace
graph export "$codex_figures/bjs_conflict_drought_fire_bin_new.pdf", replace

* -------------------------------------------------------------------
* Figure 4
* -------------------------------------------------------------------

local plotcoefs_twfe lead6 lead5 lead4 lead3 lead2 lag0 lag1 lag2 lag3 lag4 lag5 lag6 lag7 lag8
...
coefplot ///
    ...
    xtitle("Years from shock onset (tau_-1 = reference)") ///
    ytitle("Effect on log(1 + new BINs)") ///
    title("TWFE event-study: BIN new-discovery response to shock onsets", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(rows(1) position(6) size(small)) ///
    name(es_binnew_multishock_twfe, replace)
graph save "$proj/Output/figures/event_study/twfe_conflict_drought_fire_bin_new.gph", replace
graph export "$proj/Output/figures/event_study/twfe_conflict_drought_fire_bin_new.pdf", replace
graph export "$codex_figures/twfe_conflict_drought_fire_bin_new.pdf", replace
...
coefplot (`est_cont', label("DL continuous") msymbol(O) mcolor(navy) ciopts(lcolor(navy))), ///
    ...
    ytitle("Effect on log(1 + new BINs) per unit log(1 + events)") ///
    title("Continuous-treatment DL DID: conflict intensity and BIN new discoveries", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_binnew_continuous, replace)
graph save "$proj/Output/figures/event_study/continuous_dl_conflict_bin_new_log1p.gph", replace
graph export "$proj/Output/figures/event_study/continuous_dl_conflict_bin_new_log1p.pdf", replace
graph export "$codex_figures/continuous_dl_conflict_bin_new_log1p.pdf", replace
...
coefplot (`est_dcdh', label("dCDH dynamic") msymbol(O) mcolor(navy) ciopts(lcolor(navy))), ///
    ...
    ytitle("Effect on log(1 + new BINs) per unit log(1 + events)") ///
    title("dCDH continuous DID: conflict intensity and BIN new discoveries", size(medium)) ///
    msize(small) ciopts(recast(rcap)) legend(off) ///
    name(es_binnew_dcdh, replace)
graph save "$proj/Output/figures/event_study/dcdh_continuous_conflict_bin_new_log1p.gph", replace
graph export "$proj/Output/figures/event_study/dcdh_continuous_conflict_bin_new_log1p.pdf", replace
graph export "$codex_figures/dcdh_continuous_conflict_bin_new_log1p.pdf", replace

display _n "{txt}=== Done. ==="
display "Log:    $proj/Logs/reg_event_study_bin_new.log"
display "Figures:"
display "  $proj/Output/figures/event_study/twfe_vs_bjs_conflict_bin_new_log1p.pdf"
display "  $proj/Output/figures/event_study/twfe_vs_bjs_conflict_bin_new_any.pdf"
display "  $proj/Output/figures/event_study/bjs_conflict_drought_fire_bin_new.pdf"
display "  $proj/Output/figures/event_study/twfe_conflict_drought_fire_bin_new.pdf"
display "  $proj/Output/figures/event_study/continuous_dl_conflict_bin_new_log1p.pdf"
display "  $proj/Output/figures/event_study/dcdh_continuous_conflict_bin_new_log1p.pdf"
