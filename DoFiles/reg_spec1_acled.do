* reg_spec1_acled.do
* ACLED violent shocks -> BOLD sampling activity
* Table 1: Cell + Year FE (main)
* Table 2: Cell + Country×Year FE (robustness)
*
* Treatment definition:
*   acled_violent_events = battles + explosions/remote violence
*                          + violence against civilians
*   acled_any_violent    = 1[acled_violent_events > 0]
*
* This deliberately excludes protests, riots, and strategic developments from
* the main treatment because they are broader political/reporting events rather
* than direct violence/access-disruption shocks.
...
if "`panel_mode'" == "all" {
    foreach m in "100-yearly" "50-yearly" "50-quarterly" {
        di _n as text "============================================================"
        di as text "Running reg_spec1_acled.do for panel_mode = `m'"
        di as text "============================================================"
        do "`proj'/DoFiles/reg_spec1_acled.do" "`m'"
    }
    exit
}
...
if "`panel_mode'" == "100-yearly" {
    ...
    local log_path   "`proj'/Logs/reg_spec1_acled_100km_year.log"
    ...
    local log_path   "`proj'/Logs/reg_spec1_acled_50km_year.log"
    local panel_freq "year"
    ...
    local log_path   "`proj'/Logs/reg_spec1_acled_50km_quarter.log"
...
local absorb_main "cell_id_num `timevar' `absorb_cell_season'"
...
local absorb_country_time "cell_id_num country_time `absorb_cell_season'"
...
foreach v in acled_events_battles acled_events_explosions acled_events_vac {
...
gen acled_violent_events = acled_events_battles + ///
    acled_events_explosions + acled_events_vac
replace acled_violent_events = 0 if missing(acled_violent_events)
capture confirm variable acled_any_violent
if _rc {
    gen byte acled_any_violent = (acled_violent_events > 0)
}
else {
    replace acled_any_violent = (acled_violent_events > 0) if missing(acled_any_violent)
}
...
summarize any_total log1p_total acled_violent_events acled_any_violent ///
...
* -------------------------------------------------------------------
* Panel A: conflict = log(1 + ACLED violent events)
* -------------------------------------------------------------------

gen conflict = log(1 + acled_violent_events)
...
eststo t1_1: `hdfe_cmd' any_total conflict forest_loss_share ///
        ...
        log_gdp_pc protected_share log_gdp_pc_x_protected_share log_gdp_pc_sq log_gdp_pc_sq_x_protected_share, ///
...
estadd local conflict_measure "log(1+ACLED violent events)"
}
...
* -------------------------------------------------------------------
* Panel B: conflict = 1[ACLED violent events > 0]
* -------------------------------------------------------------------

drop conflict L1_conflict L2_conflict
gen conflict = acled_any_violent
gen L1_conflict = L.acled_any_violent
gen L2_conflict = L2.acled_any_violent
...
estadd local fe_cell "\checkmark"
estadd local fe_year "\checkmark"
estadd local conflict_measure "1[ACLED violent events>0]"
}
...
esttab t1_*, keep(conflict forest_loss_share burned_share cyclone earthquake ///
    ...
    varlabels(conflict "ACLED violent conflict" ///
    ...
    stats(conflict_sum_txt conflict_sum_se_txt ///
          ...
          fe_cell fe_year, ///
          labels("Sum ACLED conflict L0-L2" " " ///
                 "Sum PDSI L0-L2" " " ///
                 "Sum tmax L0-L2" " " ///
    ...
    title("Table 1: ACLED Shocks and BOLD Sampling — Cell + Year FE") ///
...
esttab t2_*, keep(conflict forest_loss_share burned_share cyclone earthquake ///
    ...
    stats(conflict_sum_txt conflict_sum_se_txt ///
          ...
          fmt(%s %s %s %s %s %s ///
              %s %9.4f %9.4f %9.0fc %9.4f %s %s)) ///
    title("Table 2: ACLED Shocks and BOLD Sampling — Cell + Country x Year FE") ///
    mtitles("Any" "log(1+N)" "Any" "log(1+N)" "Any" "log(1+N)" "Any" "log(1+N)") ///
    mgroups("Contemporaneous" "With Lags" "Contemporaneous" "With Lags", ///
            pattern(1 0 1 0 1 0 1 0)) ///
    compress
...
esttab t3_*, keep(conflict forest_loss_share burned_share cyclone earthquake ///
    ...
    title("Table 3: ACLED Shocks and BOLD Sampling — Country x Year + Biome x Year FE") ///
    mtitles("Any" "log(1+N)" "Any" "log(1+N)" "Any" "log(1+N)" "Any" "log(1+N)") ///
    mgroups("Contemporaneous" "With Lags" "Contemporaneous" "With Lags", ///
            pattern(1 0 1 0 1 0 1 0)) ///
    compress
...
esttab t3_1 t3_2 t3_3 t3_4 t3_5 t3_6 t3_7 t3_8 ///
    using "`codex_tabledir'/tab_acled_africa_t3_`mode_tag'.tex", ///
    replace fragment nomtitles noobs ///
    keep(conflict L1_conflict L2_conflict) ///
    order(conflict L1_conflict L2_conflict) ///
    se star(* 0.10 ** 0.05 *** 0.01) b(4) se(4) ///
    varlabels(conflict "ACLED violent conflict" ///
              L1_conflict "ACLED conflict (t-1)" ///
              L2_conflict "ACLED conflict (t-2)") ///
...
esttab t4_*, keep(conflict c.conflict#c.msa_overall ///
    ...
    se star(* 0.10 ** 0.05 *** 0.01) b(4) se(4) ///
    varlabels(conflict "ACLED violent conflict" ///
              c.conflict#c.msa_overall "ACLED conflict x MSA" ///
              forest_loss_share "Forest loss" ///
              burned_share "Burned area" ///
              cyclone "Cyclone (64kt+)" ///
              earthquake "Earthquake (M6+)" ///
    ...
    stats(conflict_sum_txt conflict_sum_se_txt ///
          ...
          fe_cell fe_cy fe_biome_yr road_yr, ///
          labels("Sum ACLED conflict L0-L2" " " ///
                 "Sum ACLED conflict x MSA L0-L2" " " ///
                 "Sum PDSI L0-L2" " " ///
                 "Sum tmax L0-L2" " " ///
                 "ACLED measure" "Dep. var. mean" "Dep. var. SD" ///
                 "Obs." "R-sq." "Cell FE" "Country x Year FE" ///
          ...
          fmt(%s %s %s %s %s %s %s %s ///
              %s %9.4f %9.4f %9.0fc %9.4f %s %s %s %s)) ///
    title("Table 4: ACLED Conflict x Biodiversity Intactness (MSA)") ///
    mtitles("Any" "log(1+N)" "Any" "log(1+N)" "Any" "log(1+N)" "Any" "log(1+N)") ///
    mgroups("Contemporaneous" "With Lags" "Contemporaneous" "With Lags", ///
            pattern(1 0 1 0 1 0 1 0)) ///
    compress
...
esttab t5_*, keep(conflict c.conflict#c.richness_std ///
    ...
    se star(* 0.10 ** 0.05 *** 0.01) b(4) se(4) ///
    varlabels(conflict "ACLED violent conflict" ///
              c.conflict#c.richness_std "ACLED conflict x Richness (SD)" ///
              forest_loss_share "Forest loss" ///
              burned_share "Burned area" ///
              cyclone "Cyclone (64kt+)" ///
              earthquake "Earthquake (M6+)" ///
    ...
    stats(conflict_sum_txt conflict_sum_se_txt ///
          ...
          fe_cell fe_cy fe_biome_yr road_yr, ///
          labels("Sum ACLED conflict L0-L2" " " ///
                 "Sum ACLED conflict x Rich. L0-L2" " " ///
                 "Sum PDSI L0-L2" " " ///
                 "Sum tmax L0-L2" " " ///
                 "ACLED measure" "Dep. var. mean" "Dep. var. SD" ///
                 "Obs." "R-sq." "Cell FE" "Country x Year FE" ///
          ...
          fmt(%s %s %s %s %s %s %s %s ///
              %s %9.4f %9.4f %9.0fc %9.4f %s %s %s %s)) ///
    title("Table 5: ACLED Conflict x Species Richness (IUCN)") ///
    mtitles("Any" "log(1+N)" "Any" "log(1+N)" "Any" "log(1+N)" "Any" "log(1+N)") ///
    mgroups("Contemporaneous" "With Lags" "Contemporaneous" "With Lags", ///
            pattern(1 0 1 0 1 0 1 0)) ///
    compress

esttab t5_1 t5_2 t5_3 t5_4 t5_5 t5_6 t5_7 t5_8 ///
    using "`codex_tabledir'/tab_acled_africa_t5_`mode_tag'.tex", ///
    replace fragment nomtitles noobs ///
