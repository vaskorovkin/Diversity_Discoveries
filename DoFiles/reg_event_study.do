...
*   STEP 1: TWFE event-study with leads/lags via `eventdd` (Clarke-Tapia 2022).
...
*
* SECTION B — Multi-shock parallel BJS comparison (intensity-margin only):
*   Severe drought (PDSI < -2) + fire (any_burned) onset event-studies.
*   Stacked against the SECTION A Step 2 BJS-conflict estimate to show the
*   "Dell-Jones-Olken multi-shock review" comparison at cell-year resolution.
*
...
*   K=5 for conflict and drought (slow-moving, persistent shocks).
...
foreach pkg in did_imputation did_multiplegt_dyn csdid drdid coefplot event_plot eventdd ///
...
* ---- Conflict onset (K=5): first ucdp_any_all=1 after 5 zero years ----
...
* -------------------------------------------------------------------
* A.STEP 1: TWFE event-study via `eventdd` — both LHS
*   This is the conventional pre-BJS estimator. Its weakness under
*   heterogeneous treatment timing (Goodman-Bacon 2021) is the whole
*   reason Step 2 exists; comparison to Step 2 quantifies the wedge.
* -------------------------------------------------------------------

est clear

display _n "{txt}=== STEP 1: TWFE event-study (conflict, eventdd) ==="

foreach lhs in any_total log1p_total {
    display _n "{txt}--- TWFE event-study: LHS = `lhs' ---"

    qui eventdd `lhs', timevar(et_conflict) hdfe ///
        absorb(cell_id_num country_year biome_year) ///
        vce(cluster cell_id_num) ///
        leads(5) lags(10) accum
...
estadd local treatment    "Intensity log(1+events)"
estadd local lhs_label    "log1p_total"
estadd local fe_note      "* via residualization"
...
* ===================================================================
* SECTION B — MULTI-SHOCK PARALLEL BJS COMPARISON
*   Drought + fire onset event-studies on log1p_total, stacked against
*   the Section A Step 2 BJS-conflict-log1p estimate.
* ===================================================================

display _n "{txt}=== SECTION B: Multi-shock BJS comparison ==="
...
* ===================================================================
* SECTION C — ROBUSTNESS: K=10 onset definition for conflict
*   The K=5 conflict-onset spec showed a pre-trend rejection on the
*   intensive margin (pre_p = 0.020). One hypothesis: K=5 admits
*   recently-pacified cells whose 5-year peace was actually a
*   post-conflict recovery, with residual sampling effects from the
*   prior conflict episode contaminating the pre-period. Re-run with
*   K=10 to require a longer pre-onset peace window. If the
*   intensive-margin pre_p crosses 0.10 (or at minimum improves
*   substantially from 0.020), the K=5 issue is confirmed and we
*   should swap K=10 in as the headline onset definition.
*
*   Trade-off: K=10 means earliest possible onset is 2015 (panel
*   starts 2005). Drops ~5 cohort years of onsets, reducing power.
*   Diagnose cohort sizes before interpreting the pre_p change.
* ===================================================================

display _n "{txt}=== SECTION C: K=10 onset robustness ==="

* ---- Conflict onset (K=10): first ucdp_any_all=1 after 10 zero years ----
gen lookback_conflict_k10 = 1
forvalues k = 1/10 {
    replace lookback_conflict_k10 = 0 if L`k'.ucdp_any_all != 0 | missing(L`k'.ucdp_any_all)
}
gen onset_flag_conflict_k10 = (ucdp_any_all == 1 & lookback_conflict_k10 == 1)
bysort cell_id_num (year): egen onset_year_conflict_k10 = ///
    min(cond(onset_flag_conflict_k10==1, year, .))
gen Ei_conflict_k10 = onset_year_conflict_k10

* Diagnostic: cohort sizes under K=10
preserve
bysort cell_id_num: keep if _n == 1
display _n "{txt}--- conflict onset cohorts (K=10) ---"
tab Ei_conflict_k10, missing
restore

* BJS event-study with K=10 onset, both LHS
foreach lhs in any_total log1p_total {
    display _n "{txt}--- K=10 BJS event-study: LHS = `lhs' ---"

    qui did_imputation `lhs' cell_id_num year Ei_conflict_k10, ///
        fe(cell_id_num country_year biome_year) ///
        horizons(0/10) pretrends(5) ///
        cluster(cell_id_num) autosample

    estimates store bjs_conflict_k10_`lhs'
...
* ---- Pre-trend comparison K=5 vs K=10 ----
display _n(2) "{txt}========================================================================"
display "{txt}  PRE-TREND COMPARISON: K=5 vs K=10 onset definitions"
display "{txt}========================================================================"
foreach m in bjs_conflict_log1p_total bjs_conflict_k10_log1p_total ///
             bjs_conflict_any_total bjs_conflict_k10_any_total {
    estimates restore `m'
    display %-40s "`m'" ///
            "  pre_p = " %5.3f e(pre_p) ///
            "  pre_F = " %5.2f e(pre_F) ///
...
display "{txt}========================================================================"
display "{txt}  Decision rule: if K=10 intensive-margin pre_p > 0.10 (or"
display "{txt}    substantially > K=5's 0.020), swap K=10 in as headline."
display "{txt}========================================================================"
...
*   eventdd:  lead5  lead4  lead3  lead2 | lag0  lag1  ... lag10
*             (lead1 is the reference, omitted)
*   BJS:      pre5   pre4   pre3   pre2  pre1 | tau0  tau1  ... tau10
*             (τ_{-1} is the reference, omitted)
* Mapping to common τ_{m#} / τ_{p#} naming (τ_{-1} = reference):
*   τ_{-6}: BJS pre5  (no eventdd analog with leads=5)
*   τ_{-5}: TWFE lead5 (binned)        / BJS pre4
*   τ_{-4}: TWFE lead4                  / BJS pre3
*   τ_{-3}: TWFE lead3                  / BJS pre2
*   τ_{-2}: TWFE lead2                  / BJS pre1
*   τ_{0}:  TWFE lag0                   / BJS tau0
*   ...
*   τ_{+10}: TWFE lag10 (binned)        / BJS tau10
...
esttab twfe_any_total bjs_conflict_any_total twfe_log1p_total bjs_conflict_log1p_total, ///
    rename(lead5 t_m5 lead4 t_m4 lead3 t_m3 lead2 t_m2 ///
           ...
           lag6 t_p6 lag7 t_p7 lag8 t_p8 lag9 t_p9 lag10 t_p10 ///
           pre5 t_m6 pre4 t_m5 pre3 t_m4 pre2 t_m3 pre1 t_m2 ///
           tau0 t_p0 tau1 t_p1 tau2 t_p2 tau3 t_p3 tau4 t_p4 tau5 t_p5 ///
           tau6 t_p6 tau7 t_p7 tau8 t_p8 tau9 t_p9 tau10 t_p10) ///
    keep(t_m6 t_m5 t_m4 t_m3 t_m2 ///
         t_p0 t_p1 t_p2 t_p3 t_p4 t_p5 t_p6 t_p7 t_p8 t_p9 t_p10) ///
    ...
    varlabels(t_m6 "τ_{-6}" t_m5 "τ_{-5}+" t_m4 "τ_{-4}" t_m3 "τ_{-3}" t_m2 "τ_{-2}" ///
              ...
              t_p5 "τ_{+5}" t_p6 "τ_{+6}" t_p7 "τ_{+7}" t_p8 "τ_{+8}" t_p9 "τ_{+9}" ///
              t_p10 "τ_{+10}+") ///
    stats(att_avg_03_txt att_avg_03_se_txt ///
          ...
          fmt(%s %s %s %s %9.4f %s %s %s %9.0fc %9.4f %s %s %s)) ///
    title("Table 1: Conflict identification ladder — TWFE vs BJS, two LHS") ///
...
esttab cont_conflict_log1p_total dcdh_conflict_log1p_total cs_conflict_log1p_total, ///
    keep(F5_log1p_conflict F4_log1p_conflict F3_log1p_conflict F2_log1p_conflict ///
         ...
         Tm5 Tm4 Tm3 Tm2 Tm1 ///
         Tp0 Tp1 Tp2 Tp3 Tp4 Tp5 Tp6 Tp7 Tp8 Tp9 Tp10) ///
    se star(* 0.10 ** 0.05 *** 0.01) b(4) se(4) ///
    stats(conflict_sum_03_txt conflict_sum_03_se_txt ///
          conflict_sum_05_txt conflict_sum_05_se_txt ///
          pretrend_p estimator treatment lhs_label N r2 ///
          fe_cell fe_cy fe_biome_yr fe_note, ///
          labels("Sum L0-L2" " " ///
                 "Sum L0-L4" " " ///
                 "Pre-trend joint p" "Estimator" "Treatment" "LHS" "Obs." "R-sq." ///
                 "Cell FE" "Country x Year FE" "Biome x Year FE" "FE note") ///
          fmt(%s %s %s %s %9.4f %s %s %s %9.0fc %9.4f %s %s %s %s)) ///
...
esttab bjs_conflict_log1p_total bjs_drought_log1p_total bjs_fire_log1p_total, ///
    ...
    stats(att_avg_03_txt att_avg_03_se_txt ///
          ...
          labels("Avg ATT τ0-τ2" " " ///
                 ...
                 "Cell FE" "Country x Year FE" "Biome x Year FE") ///
          fmt(%s %s %s %s %9.4f %s %s %s %9.0fc %9.4f %s %s %s)) ///
    title("Table 3: Multi-shock BJS comparison on log1p_total") ///
...
coefplot (bjs_conflict_log1p_total, label("BJS imputation") msymbol(D) mcolor(maroon) ///
    ...
    coeflabels(pre5 = "-6" pre4 = "-5" pre3 = "-4" pre2 = "-3" pre1 = "-2" ///
               ...
               tau6 = "+6" tau7 = "+7" tau8 = "+8" tau9 = "+9" tau10 = "+10+") ///
    vertical yline(0, lpattern(dash) lcolor(black)) ///
    xtitle("Years from conflict onset") ///
    ytitle("Effect on log(1 + total records)") ///
...
* -------------------------------------------------------------------
* FIGURE 4 — Continuous-treatment distributed-lag dynamic effects
* -------------------------------------------------------------------

display _n "{txt}=== FIGURE 4: Continuous-treatment dynamic effects (intensity) ==="
...
* -------------------------------------------------------------------
* FIGURE 5 — dCDH heterogeneity-robust dynamic effects (continuous treatment)
...
display _n "{txt}=== FIGURE 5: dCDH dynamic effects (intensity) ==="
